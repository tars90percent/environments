...
export const modelBenchmarkNativeTaskRecords: Record<string, NativeTaskRecord> = {
  ...
  "deepswe-abs-module-cache": publicRecord({
    publisherFormat: ["Repository issue task + tests", "代码仓库问题任务 + 测试"], domain: ["Software engineering · module resolution", "软件工程 · 模块解析"], split: ["Official open benchmark tasks", "官方开放基准任务"], sourceObject: ["abs-module-cache-flags", "abs-module-cache-flags"],
    identity: ["task slug / repository", "Stable issue slug and target repository snapshot.", "稳定问题标识及目标仓库快照。"], instruction: ["issue description", "The requested behavioral change and compatibility constraints.", "所需行为变更及兼容性限制。"], input: ["repository checkout", "The pinned source tree available for inspection and editing.", "可供检查与编辑的固定版本源代码树。"], template: ["patch", "A repository diff constrained to the supplied checkout.", "限于所提供代码仓库的补丁。"], reference: ["gold patch", "Publisher reference change retained upstream.", "保留在上游的发布方参考变更。"], grader: ["tests", "Task and regression tests executed against the submitted patch.", "针对提交补丁运行的任务测试及回归测试。"],
    process: ["Trace module-cache behavior, implement the requested flag semantics in the repository, and preserve existing behavior outside the issue scope.", "追踪模块缓存行为，在仓库中实现所需标志语义，并保持问题范围外的现有行为。"], output: ["A source-code patch that implements the issue and keeps the repository testable.", "一份实现该问题且保持仓库可测试的源代码补丁。"], grading: ["The harness applies the patch and runs task-specific and regression tests in the pinned environment.", "评测框架应用补丁，并在固定环境中运行任务专属及回归测试。"],
  }),
