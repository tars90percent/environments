...
function sciCodeRecord(problem: "lj" | "neutrino"): NativeTaskRecord {
  ...
  return {
    ...
    split: text("Public validation split · 15 problems", "公开 validation 集 · 15 个问题"),
...
export const modelBenchmarkNativeTaskRecords: Record<string, NativeTaskRecord> = {
  ...
  "ale-marketing-ab-test": publicRecord({
    publisherFormat: ["JSON task card + linked office files", "JSON 任务卡 + 外链办公文件"], domain: ["Business and finance · digital marketing", "商业与金融 · 数字营销"], split: ["Official open task cards", "官方开放任务卡"], sourceObject: ["digital_marketing_ab_test_analysis_1", "digital_marketing_ab_test_analysis_1"],
  ...
  "spreadsheetbench-v2-pepsico-valuation": publicRecord({
    publisherFormat: ["V2 dataset row + multi-sheet workbook", "V2 数据行 + 多工作表工作簿"], domain: ["Business spreadsheets · financial modeling", "商业电子表格 · 财务建模"], split: ["Official public v2 sample archive", "官方公开 v2 样例归档"], sourceObject: ["Financial_Model · 09_01", "Financial_Model · 09_01"],
    identity: ["id", "Stable workflow identifier in the v2 Financial_Model category.", "v2 Financial_Model 类别中的稳定工作流标识。"], instruction: ["instruction", "The end-to-end valuation modeling objective and preservation requirements.", "端到端估值建模目标及结构保持要求。"], input: ["spreadsheet_path", "The publisher workbook containing the source financial, assumption, ratio, and market-data tabs.", "包含源财务数据、假设、比率及市场数据工作表的发布方工作簿。"], template: ["answer_position", "The workbook ranges included in evaluation, including the valuation and supporting tabs.", "纳入评测的工作簿区域，包括估值页及配套工作表。"], reference: ["golden_response_path", "The publisher's completed reference workbook retained in the sample archive.", "保留在样例归档中的发布方完整参考工作簿。"], grader: ["spreadsheet evaluator", "The v2 harness recalculates and compares designated workbook ranges.", "v2 评测框架重新计算并比较指定工作簿区域。"],
    process: ["Inspect the supplied tabs, construct the DCF and comparable-company valuation, preserve the workbook structure, and save the completed file.", "检查给定工作表，构建 DCF 与可比公司估值，保持工作簿结构并保存完成文件。"], output: ["A completed valuation workbook with the required ranges populated and original formatting preserved.", "一份完成的估值工作簿，正确填充规定区域并保持原有格式。"], grading: ["The recalculated output ranges are compared with the publisher's golden workbook under the v2 evaluation pipeline.", "在 v2 评测流程中，将重新计算后的输出区域与发布方标准工作簿比较。"],
  }),
  ...
  "charxiv-session-accuracy-decline": publicRecord({
    publisherFormat: ["JSON chart-question record + figure", "JSON 图表问题记录 + 图像"], domain: ["Scientific chart reasoning", "科学图表推理"], split: ["Public reasoning_val split", "公开 reasoning_val 数据集"], sourceObject: ["figure_id 0", "图像 ID 0"],
