...
export const modelBenchmarkSampleContext: Record<string, { format: BenchmarkSampleTaskFormat; originalLanguage: "English" }> = {
  ...
  deepswe: { format: "repository-engineering", originalLanguage: "English" },
...
export const modelBenchmarkSamples: Record<string, BenchmarkSampleTask[]> = {
  ...
  deepswe: [publicTaskProfile({
    ...
    evaluation: ["The Harbor-style package applies tests to the exact checkout and grades behavior through its bundled test and grader files.", "Harbor 风格任务包在精确代码快照上运行测试，并通过随附测试与评分器文件评定行为。"],
