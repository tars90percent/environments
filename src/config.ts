...
export function validateConfig(): void {
  ...
  const registryValues = [config.registryDatabaseUrl, config.registryCatalogToken, config.registryReviewToken, config.registryAdminToken];
  if (registryValues.some(Boolean) && !registryValues.every(Boolean)) {
    throw new Error("DATABASE_URL, CASE_REGISTRY_CATALOG_TOKEN, CASE_REGISTRY_REVIEW_TOKEN, and CASE_REGISTRY_ADMIN_TOKEN must be set together");
  }
  for (const [name, token] of [["CASE_REGISTRY_CATALOG_TOKEN", config.registryCatalogToken], ["CASE_REGISTRY_REVIEW_TOKEN", config.registryReviewToken], ["CASE_REGISTRY_ADMIN_TOKEN", config.registryAdminToken]] as const) {
    if (token && token.length < 32) throw new Error(`${name} must contain at least 32 characters`);
  }
  const tokens = [config.registryCatalogToken, config.registryReviewToken, config.registryAdminToken].filter(Boolean);
