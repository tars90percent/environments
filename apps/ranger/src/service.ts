...
export class Service {
  private current?: {message: Message; controller: AbortController};
  ...
  async work() {
    ...
    try { await this.agent(message,controller.signal); }
    catch {
  ...
  status() { return {activeMessageId:this.current?.message.id || null,inbox:this.state.summary()}; }
}
